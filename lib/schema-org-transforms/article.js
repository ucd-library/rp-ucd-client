...
export function articleTransform(pub={}){
  ...
  if ( pub.publicationDate ) datePublished = schemaOrgUtils.asArray(pub.publicationDate)[0];
  ...
  // Get authors
  if ( pub.Authorship ) {
    author = schemaOrgUtils
      .asArray(pub.Authorship)
      .filter(authorship => authorship['vivo:rank'] !== undefined)
      .map(authorship => 
        schemaOrgUtils.asArray(authorship.relates).find(person => person['@type'].match('vcard'))
      )
