...
const {logger, redis, kafka} = rpNodeUtils;
const waitUntil = rpNodeUtils['wait-util'];
const {Consumer, Producer, utils} = kafka;
...
class Harvest {
  ...
  async request(user) {
    ...
    eventBus.emit('user-client-message', {user, state: 'queued'});
  ...
  async handleHarvestUpdate(msg) {
    ...
    eventBus.emit('user-client-message', payload);
  }
