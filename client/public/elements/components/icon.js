...
export class RpIcon extends LitElement {
  ...
  return {
    ...
    sizeIcon: {type: String},
    sizeIconSVG:  {type: String}
  };
  }
  ...
  renderIcon() {
    ...
    if (this.icon.startsWith('iron-')) {
      ...
      return this.sizeIcon ? html `<iron-icon class="${this.sizeIcon}" icon="${icon}"></iron-icon>`  :
                             html `<iron-icon class="icon" icon="${icon}"></iron-icon>`;
    }
  ...
  _renderCustomIcon(icon) {
    ...
    if (icon == 'subject') {
      return svg`<path d="M5.09,6.65a3.8,3.8,0,0,1-.47-.18L2.8,5.41A.83.83,0,1,1,3.62,4c.64.36,1.29.74,1.92,1.12A.79.79,0,0,1,5.89,6,.93.93,0,0,1,5.09,6.65Z"/>
  ...
  _isCustomIcon(icon){
    if (['rp-search', 'rp-qr', 'rp-subject'].includes(icon)) {
      return true;
    }
    return false;
  }
